        ...
        return (
            <div className="App" >
                <Link to='Edit/' style={{ textDecoration: 'none' }}>
                    <Button size='large' color="secondary" >New</Button>
                </Link>

                {this.state.list.map((blog) => {
                    return <List color="secondary" key={blog.id}>
                        <CardContent color="primary">
                            <Typography variant='h5'>{blog.title}</Typography>
                            <Typography variant='body1' color='textSecondary'>{blog.discription}</Typography>
                            <Button value={blog.id} color="default" onClick={this.deleteBlog}>Delete</Button>
                            <Link to={'/Edit/' + blog.id} style={{ textDecoration: 'none' }}>
                                <Button value={blog.id} color="primary">Edit</Button>
                            </Link>
                            <Link to={'/Show/' + blog.id} style={{ textDecoration: 'none' }}>
                                <Button value={blog.id} variant="outlined" color='secondary'>READ More</Button>
                            </Link>
                        </CardContent>
                    </List>
                })
